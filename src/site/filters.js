...
const isComponentReference = val =>
    val && val.type && val.flavour && val.variant;

const renderComponentReference = ref => {
    const subComponent = findComponent(ref.type);
    const subState = getStateFromFlavour(
        subComponent,
        ref.flavour,
        ref.variant,
    );
    ...
    return nunjucks.render(
        path.join(config.paths.ui.components, ref.type, `${ref.type}.html`),
        {
...
const loadComponentReferences = val => {
    ...
    if (isComponentReference(val)) {
        ...
        ret = val
            .map(
                v =>
                    isComponentReference(v) ? renderComponentReference(v) : v,
            )
            .join('');
    } else {
        ret = val;
